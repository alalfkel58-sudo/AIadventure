...
const translations = {
  ko: {
    ...
    introPlaceholder:
      "나는 낡은 오두막 안에서 눈을 뜬다. 창밖에서는 폭풍우가 몰아치고 있다.",
    npcLabel: "주요 등장인물 설정",
    npcCount: n => `${n}명`,
    npcNameLabel: "이름",
    npcDescLabel: "설정",
    ...
    apiKeyError:
      "Gemini API 키가 설정되지 않았습니다. 앱이 작동하려면 유효한 키를 입력해야 합니다.",
    genericError:
      "오류가 발생했습니다. 키가 유효한지 확인하고 다시 시도해주세요.",
  ...
  en: {
    title: "AI Adventure",
    tagline:
      "The AI creates a story just for you in real-time.\nYour choice becomes the story.",
    ...
    personaPlaceholder:
      "I am an adventurer who woke up in a strange forest with amnesia.",
    worldLabel: "World Setting",
    worldPlaceholder:
      "The continent of 'Arcadia', where magic and monsters coexist.",
    introLabel: "The Beginning (Intro)",
    introPlaceholder:
      "I wake up inside a shabby hut. A storm is raging outside.",
    ...
    apiKeyError:
      "Gemini API Key is not set. You must enter a valid key for the app to function.",
    genericError:
      "An error occurred. Please check if your key is valid and try again.",
  ...
  ja: {
    ...
    prevPage: "前へ",
    nextPage: "次へ",
    pageIndicator: (current, total) => `ページ ${current} / ${total}`,
    apiKeyError:
      "Gemini APIキーが設定されていません。アプリが機能するためには有効なキーを入力する必要があります。",
    genericError:
      "エラーが発生しました。キーが有効か確認してからもう一度お試しください。",
...
const handleDownloadLog = gameData => {
  if (!gameData) return
...
export default function App() {
  ...
  const handleStartAdventure = async setupData => {
    setIsLoading(true)
    setError(null)
    ...
    const prompt = `
      ...
      Now, based on these settings, describe the scene for "${
        fullGameData.intro
      }" and present the first choices.
    `
  ...
  const handlePlayerChoice = async choiceText => {
    if (!gameData) return
    setIsLoading(true)
    setError(null)
    ...
    const updatedHistory = gameData.history.concat([
  ...
  const updateGameFromAiResponse = (response, currentGameData) => {
    const updatedScenes = [
      ...currentGameData.scenes,
    ...
    const updatedGameData = {
      ...
      history: currentGameData.history.concat([
  ...
  const callGeminiAPI = async (prompt, currentGameData) => {
    if (!apiKey) {
          ...
          <GameScreen
            ...
            choices={
              gameData?.history
                ...
                .map(h => JSON.parse(h.parts[0].text).choices)
                .pop() || []
            }
            isLoading={isLoading}
      ...
      case "ended":
        return (
          <div className="screen">
            <h1>{t("theEnd")}</h1>
            <p>
              {gameData?.scenes[gameData.scenes.length - 1] || t("storyEnded")}
            </p>
            <div style={{ marginTop: "2rem", display: "flex", gap: "1rem" }}>
              <button onClick={() => handleDownloadLog(gameData)}>
                {t("downloadLog")}
              </button>
...
const ApiKeyScreen = ({ onKeySubmit, t }) => {
  ...
  return (
    <div className="screen" style={{ justifyContent: "center", gap: "1.5rem" }}>
      ...
      >
        ...
        />
        <button type="submit" disabled={!key.trim()} style={{ width: "100%" }}>
          {t("saveKey")}
        </button>
      </form>
    </div>
    ...
    >
      {hasSave && (
        <button onClick={onContinue} style={{ order: 1 }}>
          {t("continueAdventure")}
          <small
            style={{ display: "block", opacity: 0.8, fontWeight: "normal" }}
          >
            {t("lastSave")}
          </small>
        </button>
      ...
      <button
        onClick={onNewGame}
        style={{ order: 2, background: "var(--secondary-color)" }}
      >
        {t("newAdventure")}
      </button>
    </div>
...
const SetupScreen = ({ onStart, t, isLoading, error }) => {
  ...
  const handleNumNpcsChange = num => {
    setNumNpcs(num)
    const currentNpcs = data.npcs
    const newNpcs = Array.from(
      { length: num },
      (_, i) => currentNpcs[i] || { name: "", description: "" }
    )
    setData({ ...data, npcs: newNpcs })
  }
  ...
  const canStart = useMemo(() => {
    return (
      ...
      data.world &&
      data.intro &&
      data.npcs.every(n => n.name && n.description)
    )
  }, [data, isLoading])
  ...
  return (
    <form className="setup-screen" onSubmit={handleSubmit}>
      ...
      <div className="model-selection">
        <button
          type="button"
          onClick={() => setData({ ...data, model: "gemini-2.5-flash" })}
          className={data.model === "gemini-2.5-flash" ? "selected" : ""}
        >
          {t("modelFlash")}
        </button>
        <button
          type="button"
          onClick={() => setData({ ...data, model: "gemini-2.5-pro" })}
          className={data.model === "gemini-2.5-pro" ? "selected" : ""}
        >
          {t("modelPro")}
        </button>
      </div>
      ...
      <div>
        {[1, 2, 3].map(num => (
          <button
            type="button"
            key={num}
            onClick={() => handleNumNpcsChange(num)}
            className={numNpcs === num ? "selected" : ""}
      ...
      {Array.from({ length: numNpcs }).map((_, index) => (
        ...
        >
          ...
          <input
            id={`npcName${index}`}
            value={data.npcs[index]?.name || ""}
            onChange={e => handleNpcChange(index, "name", e.target.value)}
          />
          <label htmlFor={`npcDesc${index}`}>{t("npcDescLabel")}</label>
          <textarea
            id={`npcDesc${index}`}
            value={data.npcs[index]?.description || ""}
            onChange={e =>
...
const Typewriter = ({ text, onFinished, instant = false }) => {
  ...
  return (
    <div
      ...
      onClick={handleClick}
      dangerouslySetInnerHTML={{ __html: parseText(displayedText) }}
    />
  )
}
    ...
    <div className="game-screen">
      ...
      <header className="game-header">
        ...
        <div className="header-buttons">
          <button onClick={() => setActiveModal("status")} title={t("status")}>
            👤
          </button>
          <button
      ...
      <main className="story-window">
        {isLoading && scenes.length === 0 ? (
          <p className="loading-text">{t("storyLoading")}</p>
        ) : (
          <Typewriter text={scenes[currentPage] || ""} instant={!isLastPage} />
        )}
      </main>
...
const StatusModal = ({ playerStatus, onClose, t }) => {
  return (
    <div className="modal-overlay" onClick={onClose}>
      <div className="modal-content" onClick={e => e.stopPropagation()}>
        ...
        <div className="status-grid">
          ...
          <div>❤️ {playerStatus.health} / 100</div>
          <div>
            <strong>{t("location")}</strong>
          </div>
          <div>📍 {playerStatus.location}</div>
          <div>
            <strong>{t("time")}</strong>
          </div>
          <div>🕒 {playerStatus.time}</div>
        </div>
...
const SettingsModal = ({ onClose, onSave, onDownload, t }) => {
    ...
    <div className="modal-overlay" onClick={onClose}>
      <div className="modal-content" onClick={e => e.stopPropagation()}>
        ...
        <div className="settings-section">
          ...
          <button onClick={onDownload} style={{ marginTop: "0.5rem" }}>
            {t("downloadLog")}
          </button>
        </div>
        <button
